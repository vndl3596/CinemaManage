...
namespace GUI.DAO
{
    public class TicketDAO
    {
        ...
        public static DataTable GetListTicketsByShowTimeID(string showTimesID)
        {
            return DataProvider.ExecuteQuery("select * from Ve where idLichChieu = '" + showTimesID + "' and TrangThai = 1");
        }
        ...
        public static int BuyTicket(string ticketID, int type, float price, string idNV)
        {
            string query = "Update dbo.Ve set TrangThai = 1, LoaiVe = "
                + type + ", TienBanVe =" + price + ", idNV ='" + idNV + "' where id = '" + ticketID + "'";
            return DataProvider.ExecuteNonQuery(query);
        }
        public static int BuyTicket(string ticketID, int type, string customerID, float price, string idNV)
        {
            string query = "Update dbo.Ve set TrangThai = 1, LoaiVe = "+ type 
                + ", idKhachHang =N'" + customerID + "', TienBanVe =" + price +", idNV ='" + idNV + "' where id = '" + ticketID + "'";
            return DataProvider.ExecuteNonQuery(query);
        }
        public static int InsertTicket(int type, string showtimeID, string seatID, int status, float price, string staffID)
        {
            string query = "Insert into dbo.Ve(LoaiVe,idLichChieu,MaGheNgoi,TrangThai,TienBanVe,idNV) values (" + type +",'" + showtimeID + "','" + seatID + "'," + status + "," + price + ",'" + staffID + "')";
            return DataProvider.ExecuteNonQuery(query);
        }
        public static int InsertTicket(int type, string showtimeID, string seatID, string customerID, int status, float price, string staffID)
        {
            string query = "Insert into dbo.Ve(LoaiVe,idLichChieu,MaGheNgoi,idKhachHang,TrangThai,TienBanVe,idNV) values (" + type + ",'" + showtimeID + "','" + seatID + "','" + customerID + "'," + status + "," + price + ",'" + staffID + "')";
            return DataProvider.ExecuteNonQuery(query);
        }
