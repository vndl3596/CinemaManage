...
namespace GUI.DAO
{
    public class CinemaDAO
    {
        ...
        public static DataTable SearchCinemaByID(String id)
        {
            return DataProvider.ExecuteQuery("SELECT * FROM dbo.PhongChieu WHERE id = '" + id + "'");
        }

        public static DataTable SearchNumberOfSeatByIDTicket(String id)
        {
            return DataProvider.ExecuteQuery("SELECT PhongChieu.SoChoNgoi FROM Ve.idLichChieu = LichChieu.id AND LichChieu.idPhong = PhongChieu.id AND Ve.id = '" + id + "'");
        }
        public static DataTable SearchCinemaUsingInShowTime(String id)
        {
            return DataProvider.ExecuteQuery("SELECT * FROM dbo.LichChieu WHERE idPhong = '" + id + "'");
        }
        public static DataTable SearchCinemaStatusByID(String id)
        {
            return DataProvider.ExecuteQuery("SELECT TinhTrang FROM dbo.PhongChieu WHERE id = '" + id + "'");
        }
        public static DataTable SearchCinemaByShowTime(String id)
        {
            return DataProvider.ExecuteQuery("SELECT PhongChieu.* FROM dbo.PhongChieu, dbo.LichChieu WHERE LichChieu.id = '" + id + "' AND LichChieu.idPhong = PhongChieu.id");
        }
    }
}
