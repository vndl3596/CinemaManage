...
namespace GUI.DAO
{
    public class ScreenTypeDAO
    {
        ...
        public static DataTable SearchScreenTypeByID(String id)
        {
            return DataProvider.ExecuteQuery("SELECT * FROM dbo.LoaiManHinh WHERE id = '" + id + "'");
        }

        public static DataTable SearchScreenTypeUsingInCinema(String id)
        {
            return DataProvider.ExecuteQuery("SELECT * FROM dbo.PhongChieu WHERE idManHinh = '" + id + "'");
        }
        public static DataTable SearchScreenTypeUsingInFormatMovie(String id)
        {
            return DataProvider.ExecuteQuery("SELECT * FROM dbo.DinhDangPhim WHERE idLoaiManHinh = '" + id + "'");
        }
    }
}
