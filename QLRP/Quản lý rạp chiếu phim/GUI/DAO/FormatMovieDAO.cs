...
namespace GUI.DAO
{
    public class FormatMovieDAO
    {
        ...
        public static DataTable GetFormatMovieByMovieName(string movieName, string screenTypeName)
        {
            string command = "SELECT DD.id, P.TenPhim, MH.TenMH " +
                                "FROM dbo.DinhDangPhim DD, dbo.Phim P, dbo.LoaiManHinh MH " +
                                "WHERE DD.idPhim = P.id AND DD.idLoaiManHinh = MH.id AND P.TenPhim = N'" + movieName + "' AND MH.TENMH = N'" + screenTypeName + "'";
        ...
        public static DataTable SearchFMByID(String id)
        {
            return DataProvider.ExecuteQuery("SELECT * FROM dbo.DinhDangPhim WHERE id = '" + id + "'");
        }
        public static DataTable SearchMovieLengthByFormatID(String id)
        {
            return DataProvider.ExecuteQuery("SELECT Phim.ThoiLuong FROM dbo.LichChieu, dbo.Phim, dbo.DinhDangPhim WHERE idPhim = Phim.id AND DinhDangPhim.id = '" + id + "'");
        }

        public static DataTable SearchFormatMovieUsingInShowTime(String id)
        {
            return DataProvider.ExecuteQuery("SELECT * FROM dbo.LichChieu WHERE idDinhDang = '" + id + "'");
        }

        public static DataTable CheckMovieAndScreenUsing(String idPhim, String idLoaiManHinh)
        {
            return DataProvider.ExecuteQuery("SELECT * FROM dbo.DinhDangPhim WHERE idPhim = '" + idPhim + "' AND idLoaiManHinh = '" + idLoaiManHinh + "'");
        }
    }
}
